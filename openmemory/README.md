...
The UI sends dashboard API requests through its same-origin server proxy. For Docker Compose, provide the API token as a server-only `OPENMEMORY_API_TOKEN` environment variable, or mount the same token file used by `docker-compose.override.yml` as `OPENMEMORY_API_TOKEN_FILE`. Do not use a `NEXT_PUBLIC_` token variable: public-prefixed variables are included in browser assets.
